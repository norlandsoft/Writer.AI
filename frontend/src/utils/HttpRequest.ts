...
export async function SSE(url: string | URL, params: ChatResponseProps, callback: any) {
  const {sessionId, chatId} = params;
  const userId = sessionStorage.getItem("air-user-id");
  const eventSource = new EventSource(url + '?user=' + userId + '&session=' + sessionId + '&chat=' + chatId);
